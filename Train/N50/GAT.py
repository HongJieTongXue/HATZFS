...
class GraphAttentionLayer(nn.Module):
    ...
    def __init__(self, in_features, out_features, dropout, alpha, concat=True):
        ...
        # 定义可训练参数，即论文中的W和a
        self.W = nn.Parameter(torch.ones(size=(in_features, out_features)))
        nn.init.xavier_uniform_(self.W.data, gain=1.414)  # 初始化
        self.a = nn.Parameter(torch.ones(size=(2 * out_features, 1)))
...
class GAT(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        return F.softmax(x, dim=1)  # log_softmax速度变快，保持数值稳定