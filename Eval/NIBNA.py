...
def validate_top_k_coding_genes(node_importance_df: pd.DataFrame, mRNAs_data: pd.DataFrame,
                                gold_standard_cgc: pd.Series,
                                k: int) -> List[str]:
    """
...
def compute_top_k_precision_recall(node_importance_df: pd.DataFrame, mRNAs_df: pd.DataFrame,
                                   gold_standard_cgc: pd.Series,
                                   k: int = 200):
    """
...
def nibna(G: nx.Graph):
    ...
    print("Computing eigenvectors from the adjacency matrix")
    adj_eigen_vectors = compute_eigenvectors(G, n_communities)
    # 根据论文公式六计算节点重要性
    print("Computing importance of nodes in the cancer network")
...
def nibna_sort():
    ...
    action_choosed=node_importance_df['node']
    torch.save(action_choosed, "NIBNA.pt")
