...
print("初始化完毕！")
agent.meta_controller = torch.load("meta-N300-2-"+str(100)+".pt",map_location=torch.device("cuda:3"))
agent.controller = torch.load("controller-N300-2-"+str(100)+".pt",map_location=torch.device("cuda:3"))
state = -torch.ones((1,3164)).to("cuda:3")
goal_p =agent.meta_controller.forward(state)
action_p = []
for goal in range(agent.num_goal):
    sub_state_t = -torch.ones((1,agent.num_action,1)).to("cuda:3")

    action_goal_p = agent.controller.forward(sub_state_t, agent.adjs[goal])
    action_p.append(action_goal_p)

sub_station = torch.load("degree0_2_subgraph_50.pt")
res = {}
...
torch.save(res, "N50_1_allscores.pt")
...
degree_nodes = torch.load("graphid_2_degree0_40.pt")
res_last = []
for node in big_sort_chooesed:
    res_last.append(int(degree_nodes[node, 0]))
torch.save(res_last, "N50_d32_100.pt")
