...
def get_node_score2(G, nodesNeedcalcu, node_ability, degree_dic, rank):
    ...
    for node in nodesNeedcalcu:  # for ever node add the neighbor's weighted ability
        ...
        for nbr in neighbors:
            sum2 += 0.5 * node_ability[nbr] * (
                        weight[(nbr, node)] + (G[nbr][node].get('weight',1)) * len(neighbors) / degree_dic[node])
...
def HWVoteRank(G, l,lambdaa=0.1, beta=0.1):
    '''
    ...
    node_type_all=pd.DataFrame(columns=['gene','lable'])
    node_type_all['gene']=nodes
    # node_type_all['lable']=[0]*690+[1]*168+[2]*2359
    node_type_all['lable']=[0]*643+[1]*168+[2]*2353
...
def HWVoteRank_sort():
    ...
    # node_importance_df = HWVoteRank(G,3217)
    node_importance_df = HWVoteRank(G,3164)
    for i in node_importance_df:
        print(i)
    action_choosed=[]
    for i in node_importance_df:
        action_choosed+=[i[0]]
    torch.save(action_choosed, "HWVoteRank.pt")
